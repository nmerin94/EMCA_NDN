...
void installMobility(NodeContainer &c, int simulationEnd)
{
  ...
  bool test = true;
  if(test){
    Ptr<WaypointMobilityModel> wayMobility[numc];
...
int main (int argc, char *argv[])
{
  ...
  int producerId = randomNum->GetValue(0,numc-1);
 ...
 /* AnimationInterface anim ("ndn_rand_veh.xml");

  anim.SetConstantPosition (c.Get(0), 0, 5);
  anim.SetConstantPosition (c.Get(1), 10, 5);
  anim.SetConstantPosition (c.Get(2), 10, 20);
  anim.SetConstantPosition (c.Get(3), 20, 5);
  anim.SetConstantPosition (c.Get(4), 10, 11);
